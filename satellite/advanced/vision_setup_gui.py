"""
Vision Satellite Setup GUI
Configuration window for vision satellite client
"""
...
class VisionSetup:
    def __init__(self, root, on_complete=None):
        self.root = root
        self.root.title("Mina Vision Satellite Client - Setup")
    ...
    def create_widgets(self):
        ...
        title = tk.Label(
            self.root, 
            text="👁️ Mina Vision Satellite Client Setup",
            font=("Arial", 16, "bold"),
            pady=20
        )
        title.pack()
        ...
        privacy = tk.Label(
            self.root,
            text="🔒 Privacy: Vision processing happens locally on your PC.\nOnly events (motion, faces) are sent to Mina.",
            font=("Arial", 9),
            fg="green",
            justify="left"
        )
        privacy.pack(pady=(0, 10))
    ...
    def save_and_start(self):
        if self.save_config():
            ...
            if self.on_complete:
                ...
                try:
                    subprocess.Popen([sys.executable, "vision_gui.py"])
                except Exception as e:
